...
-- =============================================================================
-- Referência de lookup SCD2 (para uso nos repositórios e queries)
--
-- Para calcular indicadores de um período (start_date, end_date, sku):
--
-- SELECT
--   dss.snapshot_date,
--   dss.units_sold,
...
--   scp.tax_rate,
--   scp.amazon_fee_unit,
--   dss.units_sold * scp.unit_cost        AS cogs_day,
--   dss.units_sold * scp.prep_cost_unit   AS prep_day,
--   dss.gross_sales * scp.tax_rate        AS tax_day,
--   dss.units_sold * scp.amazon_fee_unit  AS amazon_fee_day
-- FROM daily_sales_snapshot dss
-- LEFT JOIN sku_cost_parameters scp
--   ON  scp.account_id      = dss.account_id
--   AND scp.marketplace_id  = dss.marketplace_id
--   AND scp.sku             = dss.sku
--   AND scp.valid_from     <= dss.snapshot_date
--   AND (scp.valid_to       > dss.snapshot_date OR scp.valid_to IS NULL)
-- WHERE dss.account_id      = :account_id
--   AND dss.marketplace_id  = :marketplace_id
--   AND dss.sku             = :sku          -- opcional
--   AND dss.snapshot_date  >= :start_date
--   AND dss.snapshot_date  <= :end_date;
-- =============================================================================
