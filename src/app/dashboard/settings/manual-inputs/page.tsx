      ...
      <SectionBlock title="Novo Lançamento por Período" className="border-dashed bg-background/30">
        <form action={saveManualInputs} className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-8 gap-4 items-end">
          ...
          <div className="space-y-1 lg:col-span-3">
            ...
            <input name="manual_notes" type="text" className="w-full bg-surface rounded-lg border-border text-sm focus:ring-accent/20 focus:border-accent outline-none px-3 py-2" />
          </div>
          ...
          <table className="min-w-full divide-y divide-border text-sm text-text-secondary">
            ...
            <tbody className="divide-y divide-border/50">
              {(inputs as PeriodManualInputs[] || []).map((input) => (
                <tr key={input.id} className="hover:bg-background/30 transition-colors">
